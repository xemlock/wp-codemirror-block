...
( function( $, wpcm, CodeMirror ) {
	...
	wpcm.autoLoadTheme = function (instance, theme) {
			...
			if (loaded_theme == undefined) {
				...
				svg.appendChild(use);
				instance.display.wrapper.appendChild(svg);
				// instance.display.wrapper.classList.add('loading');

				CodeMirror.on(link, "load", function () {
					// console.info('link loaded.', theme);
					// instance.display.wrapper.classList.remove('loading');
					instance.display.wrapper.querySelector('svg').remove();
				});
			}
		}
